...
import { config, permissions, tableConfig } from '../../../config';
...
class UsersList extends React.Component {
    ...
    getData = (page, rows, search) => {
        ...
        axios.get(config.apiURL + `api/v1/users?page=${page}&per_page=${rows}${searchText}`)
            .then(res => {
                if (res.data.status) {
                    this.setState({
                        data: res.data.users.data,
                        total: res.data.users.total,
                        page: res.data.users.current_page
                    })
                }
            });
    }
    ...
    editData = (e) => {
        ...
        data['roles'] = [this.state.selectedRole.id];

        axios.put(config.apiURL + `api/v1/users/${this.state.selectedRow.id}`, data).then(res => {
            if (res.status) {
                toast.notify(({ onClose }) => (
                    <div className="alert alert-success m-3">
                        <h5>Uğurlu əməliyyat!</h5>
                        <p className="mb-0">Sistem istifadəçisi redakdə edildi.</p>
                    </div>), { position: "top-right", duration: 2500 }
                );
                this.getData(this.state.page, this.state.rows);
    ...
    deleteData = () => {
        axios.delete(config.apiURL + `api/v1/users/${this.state.selectedRow.id}`)
            .then(res => {
                if (res.data.status) {
                    toast.notify(({ onClose }) => (
                        <div className="alert alert-success m-3">
                            <h5>Uğurlu əməliyyat!</h5>
                            <p className="mb-0">İstifadəçi uğurla silindi.</p>
                        </div>), { position: "top-right", duration: 2500 }
                    );
                    this.getData(this.state.page, this.state.rows);
                    this.setState({ selectedRow: null });
    ...
    render() {
        ...
        const columns = [{
            ...
            text: '',
            formatter: (cell, row) => {
                if (permissions.indexOf("permission:user-show") > -1 ||
                    permissions.indexOf("permission:user-update") > -1 ||
                    permissions.indexOf("permission:user-destroy") > -1) {
                    return (
                        <div className="text-center">
                            <UncontrolledDropdown>
                                ...
                                <DropdownMenu right>
                                    {
                                        permissions.indexOf("permission:user-show") > -1 &&
                                        <DropdownItem tag={Link} to={`/users/${row.id}`}>
                                    ...
                                    }
                                    {
                                        ...
                                        <DropdownItem onClick={() => {
                                            this.setState({
                                                ...
                                                selectedRole: row.roles?.length ? row.roles[0] : null,
                                            });
                                            this.toggleEditModal();
                                        }}>
                    ...
                    <div className="row mb-4">
                        <div className="col-md-12">
                            <div className="page-header">
                                <h1 className="font-weight-bold">İstifadəçilər</h1>
                                {
                                    permissions.indexOf("permission:user-store") > -1 &&
